...
const checkDaySelected = (day) => {
  ...
  // 매일 선택 시 처리
  if (day.code === 'every') {
    roundOption.selectedDays = daysOfWeek.value.filter((day) => day.code !== 'every')
    isEvery.value = true
    return
  }
...
const filmRatings = ref(['전체연령가', '12세 관람가', '15세 관람가', '19세 이상 관람가'])

const editorContainer = ref(null)
let quill

const toolbarOptions = [['image', 'bold', 'italic', 'link', 'strike', 'underline', 'header']]
...
const addRoundOptionToForm = () => {
  return {
    ...
    selectedDays: roundOption.selectedDays.map((day) => {
      return { code: day.code, times: day.times }
    }),
  ...
  <div class="d-flex flex-column col py-3">
    <form @submit.prevent="submitForm">
      ...
      <div class="row mb-3">
        ...
        <div class="col-sm-10 d-flex flex-column gap-2">
          <div class="d-flex flex-column gap-3">
            <div>
              <p class="fs-2 fw-bold mb-1">특정 기간 동안 회차 일괄 생성</p>
            ...
            <div class="d-flex align-items-center gap-2 text-nowrap">
              시작일:
              <input type="date" class="form-control" v-model="roundOption.startDate" /> 종료일:
              <input type="date" class="form-control" v-model="roundOption.endDate" />
            </div>

            <div class="d-flex flex-column accordion gap-2">
              <div class="fs-4 fw-bold">요일 설정(기본 값은 매일 회차가 생성됩니다.)</div>
              <div class="d-flex flex-column gap-3 mb-2">
                <div class="fs-6 fw-semibold">
                  요일 선택으로 다양한 형태의 회차를 생성할 수 있습니다.
                </div>

                <ul class="list-group list-group-flush list-group-numbered">
                  <li class="list-group-item">
                    매일 회차를 생성하고 싶다면 매일 체크박스를 체크하세요.
                  </li>
                  <li class="list-group-item">
                    매주 주말에만 공연이 진행된다면, 토요일과 일요일을 선택하세요.
                  </li>
                  <li class="list-group-item">
                    월요일 선택시 전체 기간 동안 월요일에만 공연이 진행됩니다.
                  </li>
                  <li class="list-group-item">
                    매일 회차를 생성하되, 특정 요일을 제외하고 싶다면 공연을 진행하는 요일을
                    선택하세요. 월요일과 수요일을 제외하고 싶다면 그 외의 모든 요일을 체크하시면
                    됩니다.
                  </li>
                </ul>
                <div>
            ...
            <div class="d-flex flex-column gap-4 text-nowrap">
              ...
              <ul class="list-group list-group-flush list-group-numbered">
                ...
                <li class="list-group-item">
                  <strong>모든 요일 회차 시간 동일</strong>을 체크하시면 선택한 모든 요일을 동일한
                  회차 시간으로 설정 할 수 있습니다.
                </li>
              </ul>
              <div>
      ...
      <div class="row">
        ...
        <div class="col-sm-10">
          <button type="submit" class="btn btn-light border-dark btn-lg w-100 shadow">
            등록하기
          </button>
        </div>
      </div>
